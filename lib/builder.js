...
//            result += " WHERE (" + sWheres.join( ") AND (" ) + ")";
            ...
            if (column.searchable === 'true'){
                if (requestQuery.search.value){
                    globalSearch = self.dbType === 'postgres' ?
                                   buildPostgresSearch(colName, requestQuery.search.value) :
                                   buildMySQLSearch(colName, requestQuery.search.value);
                }

                if (column.search.value){
                    columnSearch = self.dbType === 'postgres' ?
                                   buildPostgresSearch(colName, column.search.value) :
                                   buildMySQLSearch(colName, column.search.value);
                }
     ...
     */
    function buildPostgresSearch(colName, searchVal) {
     ...
     */
    function buildMySQLSearch(colName, searchVal) {
    ...
    function buildLimitPartial(requestQuery) {
        var sLimit = "";
        if (requestQuery && requestQuery.start !== undefined) {
    ...
    function buildQuery(requestQuery) {
        ...
        self.queryMap.recordsTotal = queries.length - 1;
        if (searchString) {
            queries.push(buildCountStatement(requestQuery));
            self.queryMap.recordsFiltered = queries.length - 1;
        }
    ...
    function parseResponse(queryResult) {
        ...
        if (queryResult && queryResult.length > 1) {
            var countObj = queryResult[self.queryMap.recordsTotal];
            result.recordsFiltered = result.recordsTotal = extractCount(countObj);
            if (self.queryMap.recordsFiltered) {
                var displayCountObj = queryResult[self.queryMap.recordsFiltered];
                result.recordsFiltered = extractCount(displayCountObj);
            }
