        ...
        for (var sdx = 0; sdx < requestQuery.columns.length; ++sdx) {
            ...
            if (column.searchable === 'true'){
                ...
                if (column.search.value){
                    columnSearch = self.dbType === 'postgres' ?
                                   buildILIKESearch(colName, column.search.value) :
                                   buildLIKESearch(colName, column.search.value);
                }

                if (globalSearch && columnSearch){
                    query.push('(' + globalSearch + ' AND ' + columnSearch + ')');
                }
                else if (globalSearch){
                    query.push(globalSearch);
                }
